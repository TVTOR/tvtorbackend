...
const getAllTutorsOfManager = async function (req, res) {
  try {
    const query = req.query;
    const perpage = parseInt(query.length) || 10;  // Default to 10 if not provided
    const skip = parseInt(query.start) || 0;       // Default to 0 if not provided
    const search = {};
    const order = (query.dir == 'asc' && parseInt(query.column) > 0) ? 1 : -1;
    let sort = 'createdAt';
    ...
    if (!mongoose.Types.ObjectId.isValid(tmId)) {
      return utilServices.errorResponse(res, "Invalid manager ID format", 400);
    }
    ...
    console.log('Search criteria:', search); // Debug log
    
    const total = await userService.countTutorManager(search);
    console.log('Total tutors found:', total); // Debug log
    ...
    // PERFORMANCE FIX: Process all subjects and comments in parallel
    const promises = data.map(async (tutor) => {
      const [subjects, commentdata] = await Promise.all([
        tutor.subjects ? getAllSubject(tutor.subjects) : Promise.resolve(null),
        userService.getComment(tutor._id)
      ]);
      
      tutor.subjectData = subjects || null;
      tutor.comment = commentdata ? commentdata.comment : null;
      return tutor;
    });
    ...
    console.error('getAllTutorsOfManager error:', error); // Debug log
    return utilServices.errorResponse(res, constants.DB_ERROR, 500);
  }
}
